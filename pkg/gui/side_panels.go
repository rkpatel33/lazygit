...
func (gui *Gui) sidePanelTabTitles() map[string]string {
	tr := gui.c.Tr
	// Fork customization: prefix the Files tab with the AI indicator icon when
	// AI commit message generation is enabled.
	filesTitle := tr.FilesTitle
	if gui.c.UserConfig().Git.AI.Enabled {
		filesTitle = "󰚩 " + filesTitle
	}
